...
from translator.backend.nmt.protocol import EchoServerClientProtocol

__all__ = ["load_data", "inference","quick_inference"
...
def quick_inference(ckpt, queue, hparams,scope=None):
  """
  Use for inference only no evaluation
  """  
  ...
  with tf.Session(
    ...
    ## FIXME LISTEN HERE FOR THE TEXT TO BE TRANSLATED
    utils.print_out("# Listening for inputs")
    ...
    while True:
      try:
        ...
        if isinstance(work, EchoServerClientProtocol):
          ...
          # Decode
          utils.print_out("# Start decoding")
          work.translation = nmt_utils.decode_only(
                "infer",
                loaded_infer_model,
                sess,
                subword_option=hparams.subword_option,
                ...
                tgt_eos=hparams.eos,
                num_translations_per_input=hparams.num_translations_per_input)
          utils.print_out("src: %s \n nmt: %s "% (work.message,work.translation))
          work.send_response()
